...
public class MethodSelector {
	...
	public List<List<ASTNode>> selectInstances() {
		...
		Algorithm algorithm = new CustomStandardAlgorithms()
			.getAlgorithm(resolutionMethodName, 
					properties, 
					problem);
	...
	private void computeAvgFitnessValueForObj(
			Algorithm algorithm, 
			List<List<Double>> fittests) 
	{
	...
	/**
	 * @return the solution as a Pair: 
	 * 		<binary representation, 
	 * 		 set of duplicated statement related to the solution>
	 * 		ex. <"0101...0011", 
	 * 			 ["if(var.equal(var))...", "try{var.apply()}..."]>
	 * 		
	 * 		The solution selected is the one:
	 * 			- which contain the statement that is duplicated the most
	...
	private List<List<ASTNode>> getMostFrequentStmts(Algorithm algorithm) {
		...
		if(problem instanceof CustomAbstractProblem) {
			...
			for(Map.Entry<String, String> entry : solStmts.entrySet()) {
				...
				if(value.equals(mostFreqStmtStr) &&
						StringUtils.countMatches(key, '1') < minNumOfOnes) {
					bestSolution = key;
				}
			}
		}

		List<List<ASTNode>> dupStmts = retrieveASTNodes(
											mostFreqStmt, 
											bestSolution);
	...
	private List<List<ASTNode>> retrieveASTNodes(
			List<String> mostFreqStmt, 
			String bestSolution) {	
		...
		for(int i=0; i < mostFreqStmt.size(); i++) {
			...
			for(int j=0; j < bestSolution.length(); j++) {
				if(bestSolution.charAt(j) == '1') {
					...
					List<ASTNode> stmtInMethod = method.getStatement(
													mostFreqStmt.get(i));
	...
	private void plotFitnessValues(
			List<List<Double>> fittests, 
			List<Double> iteration) {
		Plot p = new Plot();
		...
		for(int j=0; j < fittests.size(); j++) {
			Double[] fittests_obj = fittests.get(j).toArray(
										new Double[iteration.size()]);
